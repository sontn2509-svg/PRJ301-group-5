        ...
        <div class="app-container">
            ...
            <div class="main-content">
                ...
                <main class="page-content">
                    ...
                    <c:if test="${shortCount > 0}">
                        <div class="alert-card danger" style="font-size:15px;">
                            <i class="fas fa-triangle-exclamation alert-icon"></i>
                            <span><b>${shortCount}</b> nguyên liệu đang thiếu cho thực đơn hôm nay — cần nhập kho trước khi chế biến.</span>
                        </div>
                    </c:if>
                    ...
                    <div class="panel">
                        ...
                        <table>
                            ...
                            <tbody>
                                <c:forEach var="r" items="${shortageList}">
                                    <tr class="${r.belowStock ? 'row-shortage' : ''}">
                                        <td>
                                        ...
                                        <td><span class="ingredient-qty">${r.stockDisplay}</span></td>
                                        <td>${r.neededDisplay}</td>
                                        <td style="color: ${r.belowStock ? '#ef4444' : '#10b981'}; font-weight:600;">
                                            <c:choose>
                                                <c:when test="${r.belowStock}">
                                                    Thiếu ${r.shortageDisplay}
                                                </c:when>
                                                <c:otherwise>
                                                    Dư ${r.shortageDisplay}
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                ...
                                <c:if test="${empty shortageList}">
                                    <tr><td colspan="5" style="text-align: center; padding: 32px; color: #94a3b8;">Hôm nay chưa có thực đơn nào được lên, hoặc không có nguyên liệu cần tính.</td></tr>
                                </c:if>
                            </tbody>
                        </table>
                    </div>
